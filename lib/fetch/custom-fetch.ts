...
const sanitizeMatch =
  LDES_SANITIZE_CONTENTS_STRING ?? LDES_SANITIZE_CONTENTS_REGEX;
...
if (LDES_REQUESTS_PER_MINUTE > 0) {
  ...
  fetchF = (...args: FetchArgs) =>
    throttledFetch.throttled_fetch(args[0], args[1], sanitizedFetch);
} else {
...
export async function customFetch(
  ...
  return customHeadersFetch(input, init, fetchF);
}